...
双击 `DSH-Client-Setup-<版本>.exe`（当前为 `DSH-Client-Setup-0.1.1.exe`），一路下一步即可（默认装到当前用户目录，无需管理员权限），
...
**每个使用者都必须配置自己的 `DEEPSEEK_API_KEY`**，写在 `%USERPROFILE%\.dsh\.credentials.yaml` 里。

安装包**不包含任何凭据**：DSH 调用模型、挂件查询余额，用的都是**本机凭据库**里的 Key。
换一台机器安装，那里没有你的 Key，必须自己填一个；不填时客户端与 DSH 服务仍能正常启动，
但**无法真正对话**，余额与挂件会显示「未配置 DEEPSEEK_API_KEY」。

> 客户端自身**不读取** `DEEPSEEK_API_KEY`——它只在运行时从本机凭据库读取
> `client-connection/browser-session` 的签名密钥，用于给本机 DSH 服务签发访问 cookie。
> 这个签名密钥同样只存在于本机，不会随安装包分发。
